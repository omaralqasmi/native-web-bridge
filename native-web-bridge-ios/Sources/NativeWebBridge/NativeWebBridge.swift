...
public class NativeWebBridge: NSObject, WKScriptMessageHandler, UIImagePickerControllerDelegate,
    UINavigationControllerDelegate, PHPickerViewControllerDelegate
{

    private weak var webView: WKWebView?
    private var customHandlers:
        [String: ([String: Any], @escaping (Any?, String?) -> Void) -> Void] = [:]
    ...
    public init(webView: WKWebView) {
        ...
        webView.configuration.userContentController.add(
            WeakScriptMessageDelegate(self), name: "iosInterface")
    ...
    public func userContentController(
        ...
        guard message.name == "iosInterface", let b64 = message.body as? String,
            let data = Data(base64Encoded: b64),
            let json = try? JSONSerialization.jsonObject(with: data, options: []) as? [String: Any]
        else { return }

        guard let type = json["type"] as? String, let id = json["id"] as? String,
        ...
        if type == "request" || type == "command" {
            if let handler = customHandlers[action] {
                handler(payload) { resultPayload, errorMsg in
                    if type == "request" {
                        self.sendResponse(id: id, payload: resultPayload, error: errorMsg)
                    }
                }
    ...
    private func sendResponse(id: String, payload: Any?, error: String?) {
        ...
        guard let data = try? JSONSerialization.data(withJSONObject: response),
            let b64 = data.base64EncodedString() as String?
        else { return }
        DispatchQueue.main.async {
            self.webView?.evaluateJavaScript(
                "window.NativeBridgeReceiver.receiveMessage('\(b64)')", completionHandler: nil)
        }
    }
    ...
    public func registerHandler(
        action: String,
        handler: @escaping ([String: Any], @escaping (Any?, String?) -> Void) -> Void
    ) {
    ...
    private func registerDefaultHandlers() {
        ...
        registerHandler(action: "system.ui.showToast") { p, cb in
            ...
            DispatchQueue.main.async {
                if let view = self.topVC()?.view, let msg = p["message"] as? String {
                    let toast = UILabel(
                        frame: CGRect(
                            x: 20, y: view.frame.size.height - 100,
                            width: view.frame.size.width - 40, height: 40))
                    ...
                    UIView.animate(
                        withDuration: 0.5, delay: (p["duration"] as? String == "long" ? 3.5 : 2.0),
                        options: .curveEaseOut, animations: { toast.alpha = 0.0 },
        ...
        registerHandler(action: "system.ui.showAlert") { p, cb in
            DispatchQueue.main.async {
                let alert = UIAlertController(
                    title: p["title"] as? String, message: p["message"] as? String,
                    preferredStyle: .alert)
                alert.addAction(
                    UIAlertAction(title: p["buttonText"] as? String ?? "OK", style: .default) { _ in
                        cb(true, nil)
                    })
        ...
        registerHandler(action: "system.ui.showConfirm") { p, cb in
            DispatchQueue.main.async {
                let alert = UIAlertController(
                    ...
                    preferredStyle: .alert)
                alert.addAction(
                    UIAlertAction(title: p["okText"] as? String ?? "Yes", style: .default) { _ in
                        cb(true, nil)
                    })
                alert.addAction(
                    UIAlertAction(title: p["cancelText"] as? String ?? "No", style: .cancel) { _ in
                        cb(false, nil)
                    })
                self.topVC()?.present(alert, animated: true)
            }
        ...
        registerHandler(action: "system.notifications.getToken") { _, cb in
            cb(UserDefaults.standard.string(forKey: "APNSToken"), nil)
        }
        registerHandler(action: "system.notifications.setToken") { p, cb in
            if let t = p["token"] as? String { UserDefaults.standard.set(t, forKey: "APNSToken") }
            cb(true, nil)
        }
        ...
        registerHandler(action: "system.media.downloadImage") { p, cb in
            guard let urlStr = p["url"] as? String, let url = URL(string: urlStr) else {
                return cb(false, "Invalid URL")
            }
            URLSession.shared.dataTask(with: url) { data, _, error in
                guard let data = data, let image = UIImage(data: data), error == nil else {
                    return cb(false, "Download failed")
                }
        ...
        registerHandler(action: "system.share.text") { p, cb in
            DispatchQueue.main.async {
                let vc = UIActivityViewController(
                    activityItems: [p["text"] as? String ?? ""], applicationActivities: nil)
                self.topVC()?.present(vc, animated: true)
                cb(true, nil)
            }
        }
        ...
        registerHandler(action: "system.communication.openBrowser") { p, cb in
            if let urlStr = p["url"] as? String, let url = URL(string: urlStr) {
                DispatchQueue.main.async {
                    self.topVC()?.present(SFSafariViewController(url: url), animated: true)
                }
            }
            cb(true, nil)
        }
        registerHandler(action: "system.share.image") { p, cb in
            guard let b64 = p["base64Data"] as? String, let data = Data(base64Encoded: b64),
                let image = UIImage(data: data)
            else { return cb(false, "Invalid Base64") }
            DispatchQueue.main.async {
                let vc = UIActivityViewController(
        ...
        registerHandler(action: "system.share.video") { p, cb in
            guard let b64 = p["base64Data"] as? String, let data = Data(base64Encoded: b64) else {
                return cb(false, "Invalid Base64")
            }
            let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent(
                p["fileName"] as? String ?? "video.mp4")
            do {
        ...
        registerHandler(action: "system.hardware.haptic") { p, cb in
            DispatchQueue.main.async {
                let style: UIImpactFeedbackGenerator.FeedbackStyle =
                    (p["style"] as? String == "heavy")
                    ? .heavy : ((p["style"] as? String == "light") ? .light : .medium)
        ...
        registerHandler(action: "system.storage.setItem") { p, cb in
            if let k = p["key"] as? String { UserDefaults.standard.set(p["value"], forKey: k) }
            cb(true, nil)
        }
        registerHandler(action: "system.storage.getItem") { p, cb in
            if let k = p["key"] as? String {
                cb(UserDefaults.standard.object(forKey: k), nil)
            } else {
                cb(nil, nil)
            }
        }
        ...
        registerHandler(action: "system.secureStorage.setItem") { p, cb in
            guard let key = p["key"] as? String, let val = p["value"] as? String,
                let data = val.data(using: .utf8)
            else { return cb(nil, "Invalid") }
        ...
        registerHandler(action: "system.secureStorage.getItem") { p, cb in
            ...
            var item: AnyObject?
            if SecItemCopyMatching(query as CFDictionary, &item) == noErr, let d = item as? Data {
                cb(String(data: d, encoding: .utf8), nil)
            } else {
                cb(nil, nil)
            }
        }
        ...
        registerHandler(action: "system.biometrics.authenticate") { p, cb in
            ...
            if ctx.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &err) {
                ctx.evaluatePolicy(
                    ...
                    localizedReason: p["reason"] as? String ?? "Authenticate"
                ) { s, _ in cb(s, nil) }
            } else {
                cb(false, "Biometrics unavailable")
            }
        }

        // --- CLIPBOARD ---
        registerHandler(action: "system.clipboard.copy") { p, cb in
            if let t = p["text"] as? String { UIPasteboard.general.string = t }
            cb(true, nil)
        }
    }
    ...
    public func imagePickerController(
            ...
            let data = image.jpegData(compressionQuality: 0.6)
        {
            ...
            mediaCallback?(nil, "Failed")
        }
        mediaCallback = nil
    }
    public func imagePickerControllerDidCancel(_ picker: UIImagePickerController) {
        picker.dismiss(animated: true)
        mediaCallback?(nil, "Cancelled")
        mediaCallback = nil
    }
    public func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult])
    {
        ...
        provider.loadObject(ofClass: UIImage.self) { image, _ in
            if let img = image as? UIImage, let data = img.jpegData(compressionQuality: 0.6) {
                self.mediaCallback?(["base64": data.base64EncodedString()], nil)
            } else {
                self.mediaCallback?(nil, "Failed")
            }
